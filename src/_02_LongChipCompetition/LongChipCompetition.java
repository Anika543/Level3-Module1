...
public class LongChipCompetition {
	...
	private void FindTheLongestChip() {
		double x = 0; 
		int y =0; 
		String s;
    	for(int i = 0; i<theBeatles.size(); i++) {
    		for(int j = 0; j<theBeatles.get(i).getChips().size(); j++) {
    			if(x<theBeatles.get(i).getChips().get(j).getLength()) {
    				x=theBeatles.get(i).getChips().get(j).getLength();
    				y = i; 
    			}
    		}
    		
    	}
    	System.out.println(theBeatles.get(y).getName()); 
